...
- **Discriminated union results** (`{ ok: true; subject } | { ok: false; message }`):
  ...
  inside, as `vitest/no-conditional-expect` correctly flags that:
    ...
    ```
- **`.alexrc.json` is generated by `holocron setup`.** The canonical allow-list for the alex prose linter lives in `ALEX_CONFIG` in `packages/cli/src/commands/setup.ts`. To allow a new term across all repos: edit that constant, open a PR, merge, then run `holocron setup` in each consuming repo. Never edit individual repo `.alexrc.json` files by hand.
