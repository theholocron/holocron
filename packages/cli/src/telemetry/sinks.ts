/**
 * Telemetry sink interfaces — the seam between `telemetry.ts` orchestration and
 * the vendor SDKs. No `@sentry/node` / `posthog-node` import lives outside
 * {@link SentrySink} / {@link PostHogSink}; every other module talks to these
 * interfaces, so a vendor swap (or a {@link NoopErrorSink} for tests / opt-out)
 * is one file. Mirrors `@theholocron/logger`'s `Logger` interface.
 */
