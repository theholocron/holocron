...
import { upsertBranchProtection, RULESET_NAME } from "./branch-protection.js";
import { codecovContent, mergeCodecovComponents, readWorkspacePackages } from "./codecov.js";
...
// Re-export public API so importers of `../commands/setup.js` continue to work.
...
export async function runSetup(input: import("./run-step.js").RunSetupInput): Promise<import("./run-step.js").SetupReport> {
	...
	const steps: import("./run-step.js").SetupStepResult[] = [];
	...
	if (
		...
		steps.push(
			await runStep("source", "write .github/labeler.yml", dryRun, async () => {
				await source.writeRepoFile(".github/labeler.yml", workflowHeader("packages/cli/src/commands/setup/index.ts") + labelerBody);
			})
		);
		print(formatStep(steps[steps.length - 1]!));
	}
		...
		steps.push(
			await runStep("source", "write .github/dependabot.yml", dryRun, async () => {
				await source.writeRepoFile(
					...
					workflowHeader("packages/cli/src/commands/setup/index.ts") + DEPENDABOT_CONFIG
				);
			})
		);
		print(formatStep(steps[steps.length - 1]!));
	...
	if (loader.has("source")) {
		...
		steps.push(
			await runStep("source", "write .github/config.yml", dryRun, async () => {
				await source.writeRepoFile(".github/config.yml", sentimentBotConfig);
			})
		);
		print(formatStep(steps[steps.length - 1]!));
		steps.push(
		...
		steps.push(
			await runStep("source", "write .editorconfig", dryRun, async () => {
				await source.writeRepoFile(
					".editorconfig",
					workflowHeader("packages/cli/src/commands/setup/index.ts") + editorconfigBody
				);
			})
		);
		print(formatStep(steps[steps.length - 1]!));
		...
		steps.push(
			await runStep("source", "write .husky/prepare-commit-msg", dryRun, async () => {
				await source.writeRepoFile(".husky/prepare-commit-msg", prepareCommitMsg);
			})
		);
		print(formatStep(steps[steps.length - 1]!));
		{
