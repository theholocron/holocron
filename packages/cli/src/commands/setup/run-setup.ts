	...
	if (
		...
		steps.push(
			await runStep("source", "write .github/labeler.yml", dryRun, async () => {
				await source.writeRepoFile(".github/labeler.yml", workflowHeader("packages/cli/src/commands/setup/run-setup.ts") + labelerBody);
			})
		);
		print(formatStep(steps[steps.length - 1]!));
	}
	...
	if (loader.has("source")) {
		...
		steps.push(
			await runStep("source", "write .editorconfig", dryRun, async () => {
				await source.writeRepoFile(
					".editorconfig",
					workflowHeader("packages/cli/src/commands/setup/run-setup.ts") + editorconfigBody
				);
			})
		);
		print(formatStep(steps[steps.length - 1]!));
