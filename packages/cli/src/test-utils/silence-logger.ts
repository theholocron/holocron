/**
 * Vitest setup file — silence `@theholocron/logger` for the CLI suite.
 *
 ...
 * `context.test.ts` in `@theholocron/logger` and by `logger.test.ts`
 * (which overrides this mock with its own recording fake).
 */

import { vi } from "vitest";

vi.mock("@theholocron/logger", async (importOriginal) => {
	const actual = await importOriginal<typeof import("@theholocron/logger")>();
	const noop = (): void => {};
	const makeLogger = (): import("@theholocron/logger").Logger => {
