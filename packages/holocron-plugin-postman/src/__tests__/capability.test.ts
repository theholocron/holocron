...
describe('PostmanTooling.findCollectionByName', () => {
  ...
  it('returns null when no match', async () => {
    ...
    expect(
      await tooling.findCollectionByName({ workspaceId: 'ws-id', name: 'Missing' }),
    ).toBeNull()
  })
})
...
describe('PostmanTooling.importOpenApi', () => {
  ...
  it('throws clearly when Postman returns no collection on import', async () => {
    ...
    await expect(
      tooling.importOpenApi({ workspaceId: 'ws-id', spec: {} }),
    ).rejects.toThrow(/no collection on import/)
  })
})
...
describe('PostmanTooling.sync', () => {
  ...
  it('syncs env files when configured (create + update mix)', async () => {
    ...
    await writeFile(
      join(repoRoot, stagingPath),
      JSON.stringify({ name: 'staging', values: [] }),
    )
    await writeFile(
      join(repoRoot, prodPath),
      JSON.stringify({ name: 'production', values: [] }),
    )
    const { tooling, calls } = makeTooling(
      [
        { status: 200, body: { specs: [] } },
