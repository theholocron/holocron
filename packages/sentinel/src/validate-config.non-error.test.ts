...
// loadConfigFromContent only ever throws real Error instances in practice
// (it wraps every failure in ConfigFileError) — this covers the defensive
...
vi.mock("@theholocron/datapad", async (importOriginal) => {
	...
	return {
		...actual,
		loadConfigFromContent: vi.fn().mockRejectedValue("a plain string, not an Error"),
	};
});

describe("validateConfig — non-Error throw from loadConfigFromContent", () => {
	it("falls back to String(err) when loadConfigFromContent rejects with a non-Error value", async () => {
