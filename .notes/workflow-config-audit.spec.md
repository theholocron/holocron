...
```ts
workflows: ["lint", { name: "test", with: { "run-unit": true } }, { name: "release", with: { "run-build": true } }];
```
...
```ts
workflows: [{ name: "stale", with: { "exempt-issue-labels": "in-progress,wip,blocked" } }];
```
...
**Current state:** The reusable already accepts `days-before-stale` (default 30) and `days-before-close` (default 5) as `workflow_call` inputs. But the
...
```ts
workflows: [{ name: "lint", with: { "enable-auto-commit": false } }];
```
