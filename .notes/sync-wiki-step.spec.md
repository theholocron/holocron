...
The `wiki` step reads `fern/docs.yml` with `js-yaml`, replaces the
`products` key, and writes the file back. No markers needed — `products`
is a unique top-level key and the file is generated by `holocron setup`
so there are no hand-written comments to preserve.
...
logo, edit-this-page) is untouched — `js-yaml.dump()` round-trips the
parsed object faithfully.
...
6. Read fern/docs.yml with js-yaml.load()
7. Set parsed.products = the built array
8. Write back with js-yaml.dump()
```
...
`js-yaml` is already a transitive dependency in the workspace — no new
package needed. Import directly from `js-yaml`.
...
1. **Product ordering** — alphabetical by basepath is the default.
