...
<PluginHeader entry={getPlugins()["holocron-plugin-slack"]} capability="notifications" />

<Installation package={getPlugins()["holocron-plugin-slack"].package} dev />

<CapabilitiesTable rows={[{ capability: "notifications", token: "HOLOCRON_SLACK_TOKEN (slack)" }]} />
