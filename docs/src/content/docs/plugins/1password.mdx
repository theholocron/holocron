...
<PluginHeader entry={getPlugins()["holocron-plugin-1password"]} capability="vault" />

<Installation package={getPlugins()["holocron-plugin-1password"].package} dev />

<CapabilitiesTable rows={[
  { capability: "vault", token: "op CLI session (biometric on laptop; OP_SERVICE_ACCOUNT_TOKEN in CI)" },
]} />
