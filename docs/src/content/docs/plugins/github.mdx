...
<PluginHeader entry={getPlugins()["holocron-plugin-github"]} capability="source" />

<Installation package={getPlugins()["holocron-plugin-github"].package} dev />

<CapabilitiesTable rows={[
  { capability: "source", token: "HOLOCRON_ADMIN_TOKEN (github.admin)" },
  { capability: "ci", token: "HOLOCRON_READ_TOKEN (github.read)" },
  { capability: "secrets", token: "HOLOCRON_ADMIN_TOKEN (github.admin)" },
  { capability: "environments", token: "HOLOCRON_ADMIN_TOKEN (github.admin)" },
  { capability: "issues", token: "HOLOCRON_ISSUES_TOKEN (github.issues)" },
]} />
