...
<PluginHeader entry={getPlugins()["holocron-plugin-clerk"]} capability="auth" />

<Installation package={getPlugins()["holocron-plugin-clerk"].package} dev />

<CapabilitiesTable rows={[{ capability: "auth", token: "HOLOCRON_CLERK_SECRET_KEY (clerk)" }]} />
