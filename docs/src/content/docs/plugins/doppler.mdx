...
<PluginHeader entry={getPlugins()["holocron-plugin-doppler"]} capability="vault" />

<Installation package={getPlugins()["holocron-plugin-doppler"].package} dev />

<CapabilitiesTable rows={[{ capability: "vault", token: "HOLOCRON_DOPPLER_TOKEN (doppler)" }]} />
