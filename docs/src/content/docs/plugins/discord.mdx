...
<PluginHeader entry={getPlugins()["holocron-plugin-discord"]} capability="notifications" />

<Installation package={getPlugins()["holocron-plugin-discord"].package} dev />

<CapabilitiesTable rows={[{ capability: "notifications", token: "HOLOCRON_DISCORD_WEBHOOK (discord)" }]} />
