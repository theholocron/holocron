...
- The reusable `lint.yml` takes a `super-linter-env` input — a JSON map of super-linter `VALIDATE_*`/`FIX_*` keys produced by `@theholocron/astromech`'s `superLinterConfig()` and written into each repo's generated `lint` thin caller by `holocron sync`. It defaults to the org always-on baseline, so a thin caller that has not been re-synced behaves exactly as before.
