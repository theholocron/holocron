...
export default defineConfig({
  ...
  providers: {
    ...
    auth: "clerk",
    tooling: ["postman"],
    // errors and logs are env-var-activated at runtime (SENTRY_DSN / AXIOM_TOKEN).
...
| `errors`        | single      | Error tracking — env-var-activated (`SENTRY_DSN`); provider entry used by setup/doctor only |
| `logs`          | single      | Log aggregation — env-var-activated (`AXIOM_TOKEN` + `AXIOM_DATASET`); provider entry used by setup/doctor only |
