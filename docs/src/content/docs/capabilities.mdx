---
title: Capabilities
description: Reference for all 17 capability interfaces that Holocron plugins implement.
---
...
**Self-contained:** activates automatically when `HOLOCRON_SENTRY_DSN` (fallback: `SENTRY_DSN`) is present in the environment. A provider entry in `holocron.config` is only needed to enable `holocron setup` provisioning and `holocron doctor` checks — not for runtime activation.
...
Log aggregation — ships structured log output to an external service. Implemented by: [Axiom](./plugins/axiom).

**Self-contained:** activates automatically when `HOLOCRON_AXIOM_TOKEN` + `HOLOCRON_AXIOM_DATASET` (fallbacks: `AXIOM_TOKEN` + `AXIOM_DATASET`) are present in the environment. A provider entry in `holocron.config` is only needed to enable `holocron setup` provisioning and `holocron doctor` checks — not for runtime activation.

| Method            | Description                                        |
| ----------------- | -------------------------------------------------- |
| `describe()`      | Return the provider name and required env var keys |
| `whoami()`        | Verify the token by fetching the dataset info      |
| `ensureDataset()` | Create the Axiom dataset if it does not exist      |
