...
**Self-contained:** activates automatically when `SENTRY_DSN` is present in the environment. A provider entry in `holocron.config` is only needed to enable `holocron setup` provisioning and `holocron doctor` checks — not for runtime activation.
...
**Self-contained:** activates automatically when `AXIOM_TOKEN` and `AXIOM_DATASET` are present in the environment. A provider entry in `holocron.config` is only needed to enable `holocron setup` provisioning and `holocron doctor` checks — not for runtime activation.
